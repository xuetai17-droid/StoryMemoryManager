...
// =========================================================
// v0.11.9: preset-summary-first 0 API parser.
...
function fallbackEventLocalV0119(userMsg,assistantMsg,userName='') {
    ...
    return '本轮未检测到可安全压缩的独立关键事件；保留 source 供原文回查。';
}
...
function reconcileTemporalLocalV0117({currentDate=null,previousTime=null,candidateDate=null,candidateTime=null,combined='',authority='raw'}){
    ...
    if(date&&wd!=null){
        ...
        if(actual!=null&&wd!==actual){
            // v0.11.8: absolute date is canonical. Historical weekday text is known
            // to be stale/corrupted in some presets, so it must NEVER move the date.
            notes.push(`星期与绝对日期冲突，按 ${date} 的真实日历修正星期`);
        }
    }
...
async function repairTimelineGapLocalV0115() {
    ...
    if(!confirm(
        `将以 v0.11.9 本地代码处理 #${start}-#${endInclusive}（0 API）。\n`+
    ...
    try{
        ...
        if(status) status.textContent=`v0.11.9 本地代码正在处理 #${start}-#${endInclusive}（0 API）…`;
        ...
        if(status) status.textContent=remaining.length
            ? `v0.11.9 代码处理完成，但仍检测到大段缺口：#${remaining[0].start}-#${remaining[0].end}；时间校准 ${temporalRepair.timeFixed} 条、日期校准 ${temporalRepair.dateFixed} 条、星期校准 ${temporalRepair.weekdayFixed} 条${currentStateSync.time?`，当前剧情时间已同步`:''}。`
            : `v0.11.9 代码处理完成：覆盖 ${coverage.covered}/${coverage.total} 楼，生成 ${nodes.length} 个 timeline 节点${removed?`，替换旧代码节点 ${removed} 条`:''}；时间校准 ${temporalRepair.timeFixed} 条、日期校准 ${temporalRepair.dateFixed} 条、星期校准 ${temporalRepair.weekdayFixed} 条${currentStateSync.time?`，当前剧情时间已同步`:''}。`;
        ...
        console.error('[StoryMemory] v0.11.9 local code gap repair failed',e);
...
async function repairTimelineTimesLocalV0117() {
    ...
    try{
        ...
        console.error('[StoryMemory] v0.11.9 timeline time repair failed',e);
...
function panelHTML() {
    return `<div id="${PANEL_ID}" class="smm2-hidden">
      <div class="smm2-card">
        <div class="smm2-head"><div class="smm105-title-wrap"><b>剧情自动记忆</b><span class="smm105-version-badge">v0.11.9</span></div><button id="smm2_close">×</button></div>
...
function refreshNative() {
    ...
    if(gapsV0112.length){
        ...
        if(priorRangeV0115){
            ...
            if(gapStatusV0112) gapStatusV0112.textContent=`未检测到当前大段断档；已填入上一次 0 API 代码补档范围 #${priorRangeV0115.start}-#${priorRangeV0115.end}，可直接用 v0.11.9 结构化重建。`;
    ...
    if (!wrap) {
        ...
        wrap.innerHTML = `
          <div class="inline-drawer-toggle inline-drawer-header">
            <div class="smm105-title-wrap"><b>剧情自动记忆</b><span class="smm105-version-badge">v0.11.9</span></div>
...
function initializeExtension() {
    ...
    try {
        installUI();
        refresh();
        console.log('[StoryMemory] v0.11.9 loaded successfully');
    } catch (e) {
