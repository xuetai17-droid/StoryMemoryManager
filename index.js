...
// =========================================================
// v0.11.10: preset-summary-first 0 API parser.
...
function makeLocalTimelineNodesV0117(start,endInclusive,mem=M()){
    ...
    while(i<=endInclusive){
        ...
        if(records.length){
            ...
            const node={date:tr.date||null,time:ensureWeekdayTimelineTimeLocalV0119(tr.time,tr.date),event:event||'',source,__local_kind_v0115:'fallback'};
            if(!event) node.__coverage_only_v01110=true;
    ...
    return finalizeLocalNodesV0115(nodes).map(n=>{
        ...
        const x={...n};
        x.event=x.__coverage_only_v01110?'':(x.__local_kind_v0119==='preset_plot' ? cleanupPresetPlotLocalV0119(x.event,280) : compressEventLocalV0117(x.event,userName,170));
...
function priorCodeRepairAuditV0115(mem,start,endInclusive){
    return (mem?.audit||[]).some(a=>{
        if(!/^timeline_gap_code_backfill_v011[456789]$/.test(String(a?.type||''))) return false;
...
function latestCodeRepairRangeV0115(mem=M()){
    ...
    for(let i=arr.length-1;i>=0;i--){
        const a=arr[i];
        if(!/^timeline_gap_code_backfill_v011[456789]$/.test(String(a?.type||''))) continue;
        const r=Array.isArray(a?.range)?a.range:[];
        const x=Number(r[0]), y=Number(r[1]);
...
async function repairTimelineGapLocalV0115() {
    ...
    if(!confirm(
        `将以 v0.11.10 本地代码处理 #${start}-#${endInclusive}（0 API）。\n`+
        '优先直接读取预设已有 <abstract><plot> 作为 timeline（0 API，不再二次抽对白）；仅无结构化摘要时才使用保守本地兜底。\n'+
    ...
    try{
        ...
        if(status) status.textContent=`v0.11.10 本地代码正在处理 #${start}-#${endInclusive}（0 API）…`;
        ...
        target.audit.push({
            at:new Date().toISOString(),
            type:'timeline_gap_code_backfill_v0119',
        ...
        target.audit.push({
            at:new Date().toISOString(),
            type:'timeline_time_repair_v0119',
        ...
        if(status) status.textContent=remaining.length
            ? `v0.11.10 代码处理完成，但仍检测到大段缺口：#${remaining[0].start}-#${remaining[0].end}；时间校准 ${temporalRepair.timeFixed} 条、日期校准 ${temporalRepair.dateFixed} 条、星期校准 ${temporalRepair.weekdayFixed} 条${currentStateSync.time?`，当前剧情时间已同步`:''}。`
            : `v0.11.10 代码处理完成：覆盖 ${coverage.covered}/${coverage.total} 楼，生成 ${nodes.length} 个 timeline 节点${removed?`，替换旧代码节点 ${removed} 条`:''}；时间校准 ${temporalRepair.timeFixed} 条、日期校准 ${temporalRepair.dateFixed} 条、星期校准 ${temporalRepair.weekdayFixed} 条${currentStateSync.time?`，当前剧情时间已同步`:''}。`;
        ...
        console.error('[StoryMemory] v0.11.10 local code gap repair failed',e);
...
async function repairTimelineTimesLocalV0117() {
    ...
    try{
        ...
        mem.audit=Array.isArray(mem.audit)?mem.audit:[];
        mem.audit.push({at:new Date().toISOString(),type:'timeline_time_repair_v0119',range:[0,end],api_calls:0,...r,current_state_sync:currentStateSync});
        if(mem.audit.length>50) mem.audit=mem.audit.slice(-50);
        ...
        console.error('[StoryMemory] v0.11.10 timeline time repair failed',e);
...
async function repairTimelineGapLocalV0114() {
    ...
    try{
        ...
        if(status) status.textContent=remaining.length
            ...
            : `代码补档完成（0 API）：覆盖 ${coverage.covered}/${coverage.total} 楼，生成 ${nodes.length} 个 timeline 节点。`;
...
function panelHTML() {
    return `<div id="${PANEL_ID}" class="smm2-hidden">
      <div class="smm2-card">
        <div class="smm2-head"><div class="smm105-title-wrap"><b>剧情自动记忆</b><span class="smm105-version-badge">v0.11.10</span></div><button id="smm2_close">×</button></div>
...
function refreshNative() {
    ...
    if(gapsV0112.length){
        ...
        if(priorRangeV0115){
            ...
            if(gapStatusV0112) gapStatusV0112.textContent=`未检测到当前大段断档；已填入上一次 0 API 代码补档范围 #${priorRangeV0115.start}-#${priorRangeV0115.end}，可直接用 v0.11.10 结构化重建。`;
    ...
    if (!wrap) {
        ...
        wrap.innerHTML = `
          <div class="inline-drawer-toggle inline-drawer-header">
            <div class="smm105-title-wrap"><b>剧情自动记忆</b><span class="smm105-version-badge">v0.11.10</span></div>
...
function initializeExtension() {
    ...
    try {
        installUI();
        refresh();
        console.log('[StoryMemory] v0.11.10 loaded successfully');
    } catch (e) {
